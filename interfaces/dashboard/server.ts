...
function renderPerf(d){
  const el=document.getElementById('perf-chart');el.innerHTML='';
...
async function refresh(){
  ...
  // Memory table
  const mt=document.getElementById('memory-table');mt.innerHTML='';
  ...
  // Skills table
  const st=document.getElementById('skills-table');st.innerHTML='';
  ...
  gi.innerHTML='';if(graph){gi.append(h('div',null,h('span','stat',String(graph.entities||0)),h('span','stat-label',' Entities')),h('div',null,h('span','stat',String(graph.relations||0)),h('span','stat-label',' Relations')))}else gi.textContent='No graph data';
  // Telemetry
  const ti=document.getElementById('telemetry-info');ti.innerHTML='';
